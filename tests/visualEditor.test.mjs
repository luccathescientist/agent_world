...
test("renderVisualEditor chat bubble hotspot rerenders through injected callback", () => {
    ...
    "editor-chat-bubble-preview-list": {
      ...
      querySelectorAll(selector) {
        if (selector === ".chat-bubble-slot-hotspot") {
          return [{
            dataset: { role: "assistant", slot: "tl" },
            addEventListener(type, handler) {
              if (type === "click") hotspotHandlers.push(handler);
            },
          }];
        }
        if (selector === ".chat-item.preview") return [{ classList: { contains: () => false } }];
        return [];
      },
    },
    "chat-bubble-text-color": { value: "" },
  ...
  renderVisualEditor({
    ...
    getVisualLayerConfig: () => ({ cols: 3, rows: 3, label: "Floor", modeLabel: "Atlas", title: "Floor Atlas" }),
    getWorldCols: () => 3,
    getWorldRows: () => 3,
    normalizeStashPoint: (value) => value,
    populateRegionIdSelect: () => {},
    ...
    selectedChatBubbleTheme: () => ({ frame: { tl: { layer: "wall", token: "1:1" } }, textColor: "#ffffff" }),
    syncRendererCanvasSize: () => {},
  });
  assert.equal(hotspotHandlers.length, 1);
  hotspotHandlers[0]();
